...
function MortgageDropdown({ value, onChange }: MortgageDropdownProps) {
  const [isOpen, setIsOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 z-40 bg-overlay border border-border-default rounded-md shadow-lg min-w-[140px] animate-fade-in">
          <button
            onClick={() => {
              onChange(null);
              setIsOpen(false);
            }}
            className={`w-full text-left px-3 py-2 text-small hover:bg-hover transition-colors duration-fast ${
              value === null ? "text-accent-primary" : "text-text-primary"
            }`}
          >
            Все
          </button>
          <button
            ...
            className={`w-full text-left px-3 py-2 text-small hover:bg-hover transition-colors duration-fast ${
              value === "Да" ? "text-accent-primary" : "text-text-primary"
            }`}
          >
            Да
          </button>
          <button
            ...
            className={`w-full text-left px-3 py-2 text-small hover:bg-hover transition-colors duration-fast ${
              value === "Нет" ? "text-accent-primary" : "text-text-primary"
            }`}
          >
            Нет
          </button>
